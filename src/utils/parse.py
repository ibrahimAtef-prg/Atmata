...
def _infer_kind(source: str) -> str:
    if os.path.exists(source) and os.path.isfile(source):
        ...
        if ext in [".sql"]:
            return "sql"
        # Unknown extension: raise explicit error rather than silently misinterpreting
...
def _compute_profile(rows: List[Dict[str, Any]], schema: SchemaAST) -> ProfileAST:
    ...
    # missingness (sample-based only — computed on sampled rows, not full dataset)
    for f in schema.fields:
        ...
        for r in rows:
            ...
            if v is None or v == "":
                miss += 1
        missing_ratio_sample = miss / max(1, len(rows))
        prof.missingness[name] = missing_ratio_sample
...
def _main(argv: Optional[List[str]] = None) -> int:
    ...
    p = argparse.ArgumentParser(description="Aurora parse.py — parse CSV/Excel/JSON/SQL/Parquet into a normalized AST.")
