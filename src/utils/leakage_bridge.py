...
try:
    ...
    from metric_gate import enforce_all as _gate_enforce_all
    _GATE_AVAILABLE = True
except Exception:
    _GATE_AVAILABLE = False
    def _gate_enforce_all(metrics, stage, **kw):  # type: ignore
...
def _compute_local_metrics(original_df, n_samples: int, seed: Optional[int], prebuilt_synthetic_df=None) -> Dict[str, Any]:
    """
    ...
    If prebuilt_synthetic_df is supplied, use it instead of re-generating.
    """
    import numpy as np
    import pandas as pd
    ...
    # ── Step 1: Use pre-generated synthetic data or generate via marginal sampling ──
    if prebuilt_synthetic_df is not None and not prebuilt_synthetic_df.empty:
        synthetic_df = prebuilt_synthetic_df.copy()
    else:
    ...
    try:
        ...
        mi_risk = max(0.0, 2.0 * abs(auc_v - 0.5))
        raw_risk = (1.0 - ps_v) * 40.0 + mi_risk * 30.0 + dup_v * 20.0 + drft_v * 10.0
    ...
    # ── Metric gate: enforce thresholds on all scalar leakage metrics ──
    _scalar_metrics = {
        "privacy_score":            privacy_score,
        "duplicates_rate":          duplicates_rate,
        "membership_inference_auc": membership_inference_auc,
    }
    if statistical_reliability_score is not None:
        _scalar_metrics["statistical_reliability_score"] = statistical_reliability_score
    if dataset_risk_score is not None:
        _scalar_metrics["dataset_risk_score"] = dataset_risk_score

    _gate_failures: List[str] = []
    try:
        _gate_enforce_all(_scalar_metrics, stage="leakage_bridge")
    except Exception as _gf:
        # Leakage is measurement; gate failures become actionable warnings, not crashes.
        # The UI displays them to the user via uncertainty_notes.
        _gate_failures.append(f"MetricGate: {_gf}")
        notes.append(f"[ENFORCEMENT] {_gf}")
    ...
    return _safe_merge({
        ...
        "column_drift":             column_drift,
        "has_uncertainty":          not reliable or bool(_gate_failures),
        "uncertainty_notes":        notes,
        "error":                    "; ".join(_gate_failures) if _gate_failures else None,
        "_mode":                    "local",
...
def main(argv=None):
    ...
    p.add_argument("--synthetic",     default=None, help="Path to pre-generated synthetic CSV (skips re-generation).")
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    reid_risk = compute_reidentification_risk(original_df)
                    col_ranking = rank_sensitive_columns(original_df, pii_findings=[], column_drift=column_drift)
                    outlier_threats = detect_outlier_risk(original_df)
                    pii_dens = 0.0
                    dir_score = compute_dataset_intelligence_risk(
                        dataset_risk_score=output.get("dataset_risk_score") or 0.0,
                        reidentification_risk=reid_risk,
                        pii_density=pii_dens,
                        outlier_risk=outlier_threats,
                        privacy_score=output.get("privacy_score") or 0.5,
                    )
                    recs = generate_privacy_recommendations(
                        reidentification_risk=reid_risk,
                        pii_density=pii_dens,
                    ...
                    output["reidentification_risk"] = reid_risk
                    output["sensitive_column_ranking"] = col_ranking
                    output["outlier_risk"] = outlier_threats
                    output["dataset_intelligence_risk"] = dir_score
                    output["privacy_recommendations"] = recs
                except Exception:
                    pass
    ...
    try:
        prebuilt_synthetic_df = None
        if args.synthetic and os.path.isfile(args.synthetic):
            try:
                import pandas as _pd
                prebuilt_synthetic_df = _pd.read_csv(args.synthetic)
            except Exception:
                prebuilt_synthetic_df = None
        output = _compute_local_metrics(original_df, args.n, args.seed, prebuilt_synthetic_df=prebuilt_synthetic_df)
        print(json.dumps(output, ensure_ascii=False, default=str))
        return 0
    except Exception as exc:
        traceback.print_exc(file=sys.stderr)
