...
from metric_semantics import enforce_metric


# ==================================================================
# Shared canonical-row helper (used by DuplicatePreFilter and
# leakage_bridge.py to ensure consistent duplicate detection).
# ==================================================================

def _canonical_row(row: Dict[str, Any]) -> str:
    """
    Produce a stable canonical JSON string for a row dict.

    Numeric values are rounded to 4 decimal places so that minor
    floating-point noise does not create false non-matches.
    None / NaN are both normalised to JSON null.
    Keys are sorted so insertion order does not affect the result.
    """
...
class RowQualityFilter:
    ...
    def evaluate(self, df: pd.DataFrame) -> np.ndarray:
        ...
        # ---- Check A: IQR outer fence (contributes at most 1 failure) ----
        check_a = np.zeros(n, dtype=bool)
        for col, (lo_f, hi_f) in self._iqr_fences.items():
            ...
            check_a  = check_a | outside
        fail_count += check_a.astype(int)

        # ---- Check B: Mahalanobis coherence (contributes at most 1 failure) ----
        check_b = np.zeros(n, dtype=bool)
        for (col_a, col_b), params in self._pair_params.items():
            ...
            check_b = check_b | (valid & (d2 > _MAHAL_THRESHOLD))
        fail_count += check_b.astype(int)

        # ---- Check C: Conditional label plausibility (contributes at most 1 failure) ----
        check_c = np.zeros(n, dtype=bool)
        bl = self.bl
        if (bl.label_col
            ...
            for col, class_stats in bl.class_numeric_stats.items():
                ...
                for lv, stats in class_stats.items():
                    ...
                    check_c  = check_c | outside
        fail_count += check_c.astype(int)
    ...
    def summary(self, mask: np.ndarray) -> Dict[str, Any]:
        ...
        return {
            ...
            "acceptance_rate": enforce_metric("acceptance_rate", n_accepted / max(n_total, 1)),
        }
...
class ValidationLayer:
    ...
    def run(
        ...
        df, n_repaired_constraints = self._cf.apply(df)
        if n_repaired_constraints > 0:
            warnings.append(
                f"ConstraintFilter: repaired {n_repaired_constraints} out-of-range / "
        ...
        return ValidationResult(
            ...
            n_rejected_constraints = n_repaired_constraints,
