...
def build_baseline(
    ...
    *,
    ...
    meta.col_order = list(df.columns)
    ...
    if max_null_ratio > 0.0:
        for col, nr in null_ratios.items():
            if nr > max_null_ratio:
                meta.warnings.append(
                    f"Column '{col}' null ratio {nr:.3f} exceeds max_null_ratio {max_null_ratio:.3f}."
                )
...
def _is_numeric_dtype(s: any) -> bool:
    ...
    if ratio < 0.95:
        return False
    # Guard: high uniqueness on object dtype = likely an ID column, not numeric
